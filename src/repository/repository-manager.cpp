...
RepositoryManager::RepositoryManager(core::LoggerManager *loggerManager,
...
void RepositoryManager::FindRepositories() {
  this->repositories.clear();
  int count = 1;
  if (!std::filesystem::exists(this->repositoriesPath) ||
      !std::filesystem::is_directory(this->repositoriesPath)) {
    if (!std::filesystem::create_directory(this->repositoriesPath)) {
      throw std::runtime_error("Failed to create directory " +
                               this->repositoriesPath.generic_string());
    }
  }
  for (const auto &entry :
       std::filesystem::directory_iterator(this->repositoriesPath)) {
    if (entry.is_regular_file() || entry.is_symlink()) {
      try {
        ...
        if (name.empty())
          throw std::runtime_error("Repository name at '" +
                                   entry.path().generic_string() +
                                   "' cannot be empty");
        if (url.empty())
          throw std::runtime_error("Repository url at '" +
                                   entry.path().generic_string() +
                                   "' cannot be empty");
        ...
        AddRepository(
            RepositoryConfig{enabled, name, url, supportsCompression});
...
void RepositoryManager::AddRepository(RepositoryConfig config) {
  if (ContainsRepository(config.name)) {
    throw std::runtime_error("A repository with the name '" + config.name +
                             "' already exists");
  }
  std::filesystem::path configPath =
      this->repositoriesPath / (config.name + ".toml");
  core::Logger *repoLogger = this->loggerManager->GetLogger();
  Repository *repository =
      new Repository(repoLogger, config, configPath, this->cachePath);
...
const std::list<Repository *> &RepositoryManager::GetRepositories() {
